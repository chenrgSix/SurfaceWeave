...
- **自定义整页配色：** 不局限于四个主题；天空蓝或其他六位颜色值经语义令牌校验，由可信组件统一渲染。
- **结构随对话迁移：** 菜单移动到左、右、上、下；模型可新增展示卡片、指标、文本和网格，重建整页 Surface 树。
- **业务组件进化：** 普通选择框变成方案对比卡，审批区前置；所有已输入数据保留。
- **随时组合与撤销：** 连续叠加模板、打开共享视图、撤销最近变更；恢复结构而非覆盖最新输入。
- **真实执行可核查：** 自由描述布局或整页重建，查看模型原始工具参数、SDK 结果与前后 Surface 树。模型文字不冒充执行成功，生成指标不冒充业务事实。
...
layout, full page regeneration, component replacement, and undo use validated SDK tools. Model operations cannot submit business actions or execute arbitrary code.
